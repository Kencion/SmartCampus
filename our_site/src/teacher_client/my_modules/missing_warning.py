...
def get_data_update():
    ...
    for student_num in student_nums:
        try:
            ...
            student = Student(student_num=student_num)
        student.student_name = Search_student_info(student_num)['姓名']
        student.is_missing = True
        student.missing_reason = missing_students[student_num]
        student.save()
...
def get_missing_students():
    ...
    """
    missing_students = [[i.student_num, i.student_name, i.missing_reason]
                        for i in Student.objects.filter(is_missing__exact=True)]
