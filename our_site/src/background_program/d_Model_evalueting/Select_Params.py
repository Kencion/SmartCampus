...
class Select_Params(object):
    ...
    """
    def cross_val_score_model(self,estimator,X,Y,cv,scoring="accuracy"):
#         print(scoring)
        kfold = model_selection.KFold(n_splits=10, random_state=1)
        rkfold=model_selection.RepeatedKFold(n_splits=10, random_state=1)
        results = model_selection.cross_val_score(estimator,X,Y,cv=kfold,scoring=scoring)#cv一定要有值
        results2 = model_selection.cross_val_score(estimator,X,Y,cv=rkfold,scoring=scoring)
